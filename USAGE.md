...
3. **`<PROJECT_PATH>` value:** when creating a new project, the path
   will be auto-generated by taking the project name and turning it
   into a kebab case name. Basically:

   - `ThisProject` will be create at `this-project` relative path
   - `Java2Project` will be create at `java2-project` relative path
   - you get it...

   But you can specify your project path name by using the `--path`
   or `-P` flag. When running
   `kojamp new KotlinGuy -K kotlin -P some-name`, the project output
